...
public class RipristinoDati {
    ...
    private static Visita creaArchivio(Element elemento, Elenco<TipoVisita> elencoTipiVisita) {
        LocalDate dataVisita = LocalDate.parse(elemento.getElementsByTagName("dataVisita").item(0).getTextContent());
        String titolo = elemento.getElementsByTagName("titolo").item(0).getTextContent();
        TipoVisita tipo = elencoTipiVisita.getElementByKey(titolo);
        StatiVisita stato = StatiVisita.valueOf(elemento.getElementsByTagName("stato").item(0).getTextContent());

        HashMap<Fruitore, Integer> iscrizioni = new HashMap<>();
        NodeList iscrizioniNodes = elemento.getElementsByTagName("iscrizioni");
        Element iscrizioniElement = (Element) iscrizioniNodes.item(0);
        NodeList singolaIscrizioneNodes = iscrizioniElement.getElementsByTagName("iscrizione");

        for (int i = 0; i < singolaIscrizioneNodes.getLength(); i++) {
            Element iscrizioneElement = (Element) singolaIscrizioneNodes.item(i);
            String fruitoreUsername = iscrizioneElement.getElementsByTagName("fruitore").item(0).getTextContent();
            int numPersone = Integer.parseInt(iscrizioneElement.getElementsByTagName("numPersone").item(0).getTextContent());
            Fruitore fruitore = DatiCondivisi.getFruitori().getElementByKey(fruitoreUsername);
            if (fruitore != null) {
                iscrizioni.put(fruitore, numPersone);
            }
        }

        int iscritti = Integer.parseInt(elemento.getElementsByTagName("iscritti").item(0).getTextContent());

        return new Visita(dataVisita, tipo, stato, iscrizioni, iscritti);
    }
    ...
    public static void datiCondivisi() throws Exception {
        ...
        for(Visita v : DatiCondivisi.getVisite().getElementByKey("0").getVisite().getElenco().values()){
            ...
            for(Fruitore f : v.getIscrizioni().keySet()) {
                ...
                System.out.println(fruitori.contiene(f.toString()));
